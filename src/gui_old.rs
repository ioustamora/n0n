...
struct Settings {
    ...
    chunk_size_mb: u32,
    output_dir: String,
    storage_backend: usize,
    sftp_host: String,
    sftp_user: String,
    sftp_base: String,
    sftp_mailbox_id: String,
    sftp_private_key: String,
    sftp_host_fingerprint_sha256_b64: String,
    sftp_require_host_fp: bool,
    skip_hidden: bool,
...
impl Settings {
    fn from_app(app: &AppState) -> Settings {
        Settings {
    ...
    fn apply_to_app(self, app: &mut AppState) {
...
impl eframe::App for AppState {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                if ui.button("Save Settings").clicked() {
                    let s = Settings::from_app(self);
                    if let Some(path) = rfd::FileDialog::new()
                        .add_filter("JSON", &["json"]).set_file_name("n0n-settings.json").save_file() {
                        match std::fs::write(&path, serde_json::to_vec_pretty(&s).unwrap_or_default()) {
                            Ok(_) => self.log(&format!("Saved settings to {}", path.display())),
                            Err(e) => self.log(&format!("Save settings error: {e}")),
                        }
                    }
                }
                if ui.button("Load Settings").clicked() {
                    if let Some(path) = rfd::FileDialog::new()
                        .add_filter("JSON", &["json"]).pick_file() {
                        match std::fs::read(&path).ok().and_then(|b| serde_json::from_slice::<Settings>(&b).ok()) {
                            Some(s) => { s.apply_to_app(self); self.log(&format!("Loaded settings from {}", path.display())); },
                            None => self.log("Failed to load settings"),
                        }
                    }
                }
            });
    ...
    if self.storage_backend == 1 {
            ...
            ui.label("Note: defaults to recipient key string if empty.");
                    if ui.button("Test SFTP Connection").clicked() {
                        ...
                        std::thread::spawn(move || {
                            ...
                            let res = if !pk.is_empty() || !host_fp.is_empty() {
                                storage::test_sftp_connection_auth(
                                    &host,
                                    &user,
                                    if pass.is_empty() { None } else { Some(pass.as_str()) },
                                    if pk.is_empty() { None } else { Some(pk.as_str()) },
                                    if pk_pass.is_empty() { None } else { Some(pk_pass.as_str()) },
                                    if host_fp.is_empty() { None } else { Some(host_fp.as_str()) },
                                    &base,
                                )
                            } else {
                            ...
                            if let Ok(mut l) = logs.lock() {
                                match res {
                                    Ok(()) => l.push("SFTP connection OK".to_string()),
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button("Estimate Chunks").clicked() {
                    let logs = self.logs.clone();
                    let folder = self.selected_folder.clone();
                    let file = self.selected_file.clone();
                    let chunk_mb = self.chunk_size_mb;
                    let skip_hidden_flag = self.skip_hidden;
                    let est_arc = Arc::new(AtomicUsize::new(0));
                    self.estimated_total_chunks = Some(est_arc.clone());
                    // run in background
                    std::thread::spawn(move || {
                        let chunk_bytes = (chunk_mb as usize) * 1024 * 1024;
                        ...
                        if let Some(f) = file {
                            if let Ok(meta) = std::fs::metadata(&f) {
                                total = utils::estimate_chunks(meta.len() as usize, chunk_bytes);
                            }
                        } else if let Some(dir) = folder {
                        ...
                        est_arc.store(total, Ordering::Relaxed);
                        if let Ok(mut l) = logs.lock() { l.push(format!("Estimated total chunks: {total}")); }
                    });
                }
            });

            if let Some(est) = &self.estimated_total_chunks {
                let v = est.load(Ordering::Relaxed);
                ui.label(format!("Estimated total chunks: {}", v));
            }
                    ...
                    if self.watcher_running {
                        ...
                        for p in ev.paths {
                            ...
                            if skip_hidden_flag && is_hidden_path(&p) { continue; }
                                                    ...
                                                    } else if !sftp_pk.is_empty() || !sftp_host_fp.is_empty() {
                                                        let pw_opt = if sftp_pass.is_empty() { None } else { Some(sftp_pass.as_str()) };
                                                        let pk_opt = if sftp_pk.is_empty() { None } else { Some(sftp_pk.as_str()) };
                                                        let pk_pass_opt = if sftp_pk_pass.is_empty() { None } else { Some(sftp_pk_pass.as_str()) };
                                                        let hostfp_opt = if sftp_host_fp.is_empty() { None } else { Some(sftp_host_fp.as_str()) };
